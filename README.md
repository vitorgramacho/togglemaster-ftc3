...
2. Workflow `cicd-services.yml` roda os 5 estágios DevSecOps **só** para
   o serviço modificado (matriz paralela, paths-filter)
...
│   ├── cicd-services.yml       ← UM workflow matrix para os 5 serviços
...
2. **`cicd`** — `strategy.matrix` com 5 entradas (`fail-fast: false`).
   Cada entrada que tem mudança roda o reusable; as outras ficam **Skipped**
3. **`ci-summary`** — agrega resultado; é o único job que você deve
...
- **Workflow consolidado.** Um arquivo `cicd-services.yml` substitui
  cinco `ci-<service>.yml`. Matrix paralela com paths-filter — única
